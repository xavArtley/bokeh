...
p_test_vbar_x_d = figure(y_axis_type='log', title="default")
p_test_vbar_x_d.vbar(
    x=[1, 2, 3], 
    top=[1, 10, 100], 
    width=0.9,
)

p_test_circle = figure(y_axis_type='log', title="infinity circle (points at x=1 and x=3 are 0")
p_test_circle.circle(x=[1, 2, 3, 4], y=[0, 1, 0, 2], size=10)

show(
    gridplot([p_test_vbar_x_a, p_test_vbar_x_b, p_test_vbar_x_c, p_test_vbar_x_d, p_test_circle], ncols=3)
)